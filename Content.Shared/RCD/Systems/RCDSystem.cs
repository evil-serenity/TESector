...
public sealed class RCDSystem : EntitySystem
{
    ...
    private void OnDoAfterAttempt(EntityUid uid, RCDComponent component, DoAfterAttemptEvent<RCDDoAfterEvent> args)
    {
        ...
        if (!IsRCDOperationStillValid(uid, component, gridUid, mapGrid, tile, position, args.Event.Target, args.Event.User))
            args.Cancel();
    }
    ...
    private void OnDoAfter(EntityUid uid, RCDComponent component, RCDDoAfterEvent args)
    {
        ...
        // Ensure the RCD operation is still valid
        if (!IsRCDOperationStillValid(uid, component, gridUid, mapGrid, tile, position, args.Target, args.User))
            return;
