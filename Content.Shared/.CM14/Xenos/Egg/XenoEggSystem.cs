...
public sealed class XenoEggSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        // Ovipositor cooldown clear on hijack no longer used.
        ...
        // Queen lay egg action
        SubscribeLocalEvent<XenoComponent, XenoLayEggActionEvent>(OnXenoLayEggAction);
        ...
        // Disabled: interact-using and return-parasite flows
        ...
        // Disabled: step trigger infection
    ...
    private void OnXenoLayEggAction(Entity<XenoComponent> xeno, ref XenoLayEggActionEvent args)
    {
        if (args.Handled)
            return;
        ...
        // Spawn an egg item at the queen's feet; hive is set to match the queen.
        var coords = _transform.GetMoverCoordinates(xeno.Owner);
        var egg = Spawn("XenoEgg", coords);
    ...
    private void OnXenoEggPlaceDoAfter(Entity<XenoEggComponent> egg, ref XenoEggPlaceDoAfterEvent args)
    {
        ...
        // No plant sound in simplified flow
    }
    ...
    private void OnXenoEggActivateInWorld(Entity<XenoEggComponent> egg, ref ActivateInWorldEvent args)
    {
        ...
        // Require a xeno with hands to open.
    ...
    private bool CanTrigger(EntityUid user) => false;

    public bool Open(Entity<XenoEggComponent> egg, EntityUid? user, out EntityUid? spawned)
    {
        spawned = null;

        // Simplified: no special handling for parasite interactions
        ...
        // Directly spawn the creature from the egg
        if (_net.IsClient)
        {
            spawned = null;
            return true;
        }
        spawned = Spawn(egg.Comp.Spawn, _transform.GetMoverCoordinates(egg.Owner));
    ...
    public override void Update(float frameTime)
    {
        // Only the server advances egg timers/state.
        if (_net.IsClient)
            return;

        var time = _timing.CurTime;
        var eQuery = EntityQueryEnumerator<XenoEggComponent, TransformComponent>();
        while (eQuery.MoveNext(out var uid, out var egg, out var xform))
        {
            if (!xform.Anchored)
                continue;
