    ...
    private void OnBeforeInteract(Entity<DoorRemoteComponent> entity, ref BeforeRangedInteractEvent args)
    {
        ...
        switch (entity.Comp.Mode)
        {
            ...
            case OperatingMode.ToggleOvercharge:
                if (TryComp<ElectrifiedComponent>(args.Target, out var electrifiedComp)) // HardLight: Corrected multiple instances of eletrifiedComp to electrifiedComp.
                {
                    _electrify.SetElectrified((args.Target.Value, electrifiedComp), !electrifiedComp.Enabled);
                    var soundToPlay = electrifiedComp.Enabled
