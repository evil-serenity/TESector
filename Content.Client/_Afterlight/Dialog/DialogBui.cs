...
{
    private ALDialogWindow? _window;

    protected override void Open()
    {
        base.Open();
        _window = this.CreateWindow<ALDialogWindow>();
        Refresh();
    }
    ...
    private void UpdateOptions(DialogComponent s)
    {
        ...
        if (_window.Container is not ALDialogOptionsContainer container)
        {
            _window.Container?.Orphan();
            _window.Container = null;

            container = new ALDialogOptionsContainer();
    ...
    private void UpdateInput(DialogComponent s)
    {
        ...
        if (_window.Container is not ALDialogInputContainer container)
        {
            _window.Container?.Orphan();
            _window.Container = null;

            container = new ALDialogInputContainer();
    ...
    private void UpdateConfirm(DialogComponent s)
    {
        ...
        if (_window.Container is not ALDialogConfirmContainer container)
        {
            _window.Container?.Orphan();
            _window.Container = null;

            container = new ALDialogConfirmContainer();
            container.CancelButton.OnPressed += _ => Close();
    ...
    private void OnInputTextChanged(ALDialogInputContainer container, int textLength, int max)
    {
