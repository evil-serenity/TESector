    ...
    public sealed class NyanoChatSystem : EntitySystem
    {
        ...
        private List<INetChannel> GetDreamers(IEnumerable<INetChannel> removeList)
        {
            var filtered = Filter.Empty()
                .AddWhereAttachedEntity(entity => HasComp<SleepingComponent>(entity) || HasComp<SeeingRainbowsStatusEffectComponent>(entity) && !HasComp<PsionicsDisabledComponent>(entity) && !HasComp<PsionicInsulationComponent>(entity)) // HardLight: SeeingRainbowsComponent<SeeingRainbowsStatusEffectComponent
                .Recipients
                .Select(p => p.Channel);
