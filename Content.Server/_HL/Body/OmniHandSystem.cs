...
{
    public sealed class OmniHandSystem : EntitySystem
    {
        ...
        public override void Initialize()
        {
            ...
            SubscribeLocalEvent<OmniHandComponent, OmniHandToggledEvent>(OnOmniHandToggled);

            SubscribeLocalEvent<OmniHandComponent, ComponentStartup>(OnStartup);
            SubscribeLocalEvent<OmniHandComponent, ComponentShutdown>(OnShutdown);
        }

        private void OnOmniHandToggled(EntityUid uid, OmniHandComponent component, OmniHandToggledEvent args)
        {
        ...
        private void OnStartup(EntityUid uid, OmniHandComponent component, ComponentStartup args)
        {
            _actions.AddAction(uid, ref component.Action, component.ActionPrototype);
        }

        private void OnShutdown(EntityUid uid, OmniHandComponent component, ComponentShutdown args)
        {
            _actions.RemoveAction(uid, component.Action);
        }

        public bool TryToggleItem(EntityUid uid, EntProtoId proto, OmniHandComponent comp, out EntityUid? equipment)
        {
            equipment = null;
