...
public sealed class VampirismSystem : EntitySystem
{
    ...
    private void OnInitVampire(Entity<VampirismComponent> ent, ref MapInitEvent args)
    {
        ...
        if (!TryComp<BodyComponent>(ent, out var body)
		    || !_body.TryGetBodyOrganEntityComps<MetabolizerComponent>((ent, body), out var comps)) // Hardlight - body.TryGetBodyOrganComponents replaced with body.TryGetBodyOrganEntityComps because thats the system we still have
            return;

        foreach (var (_, metabolizer, organ) in comps) // Hardlight - owner placeholder fix for our systems
        {
